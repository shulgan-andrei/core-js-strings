...
function sumOfCodes(str = '') {
  ...
  return str.split('').reduce((acc, n) => acc + n.charCodeAt(0), 0);
}
...
function encodeToRot13(str) {
  ...
  const data2 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let res = '';
  for (let i = 0; i < str.length; i += 1) {
    const index = data1.indexOf(str[i]);
    if (index < 0) {
      res += str[i];
    } else {
      res += data2[index];
